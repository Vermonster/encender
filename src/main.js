 ...
 */
 export async function applyActivity(activityDefinition, patientReference=null, resolver=null, aux={}) {
  ...
  // NEW: Implement behavior to support CPG Collect Information Task
  if (activityDefinition.kind === 'Task') {
    const collectWithExtension = activityDefinition.extension.find(e => e.url.endsWith('cpg-collectWith'))

    if (collectWithExtension !== undefined) {
      targetResource.input = [
        {
          type: {
            coding: [
          ...
          valueCanonical: collectWithExtension.valueCanonical
        }
      ];
    }
  }
